...
use crate::{generate_merkle_proof_and_root, AsbifrostOptimizedBlock};
use bifrost_core::{CircuitInput, CircuitPublicValues};
...
pub fn build_transaction_proof_input(
    ...
    let bifrost_optimized_blocks = &blocks
        .iter()
        .zip(safe_block_height..safe_block_height + blocks.len() as u64)
        .map(|(block, height)| block.as_bifrost_optimized_block(height))
        .collect::<Vec<_>>();

    let chainworks = get_chainworks(bifrost_optimized_blocks, safe_chainwork)
        .iter()
        .map(|x| x.to_be_bytes())
        .collect();
    ...
    CircuitInput::new(
        ...
        bifrost_optimized_blocks.to_vec(),
        retarget_block.as_bifrost_optimized_block(retarget_block_height),
    )
}
