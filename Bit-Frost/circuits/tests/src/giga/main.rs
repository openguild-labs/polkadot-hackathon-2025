...
mod tests {
    ...
    use bifrost_core::lp::{compute_lp_hash, encode_liquidity_providers, LiquidityReservation};

    use bifrost_core::btc_light_client::AsLittleEndianBytes;
    use bifrost_core::{validate_bifrost_transaction, CircuitInput, CircuitPublicValues};
    use bifrost_lib::transaction::serialize_no_segwit;
    use bifrost_lib::{
        generate_merkle_proof_and_root, get_retarget_height_from_block_height, load_hex_bytes,
        AsbifrostOptimizedBlock,
    };
    ...
    fn get_test_case_circuit_input() -> CircuitInput {
        ...
        let chainworks: Vec<_> = mined_blocks
            .iter()
            .map(|block| block.as_bifrost_optimized_block_unsafe())
        ...
        // This is a real mainnet transaction so we don't need to build bifrost payment transaction
        ...
        CircuitInput::new(
            ...
            mined_blocks
                ...
                .map(|block| block.as_bifrost_optimized_block_unsafe())
                .collect(),
            mined_retarget_block.as_bifrost_optimized_block_unsafe(),
        )
    }

    #[test]
    fn test_mainnet_bifrost_txn() {
        validate_bifrost_transaction(get_test_case_circuit_input());
    }
    ...
    fn test_mainnet_block_proof() {
        use bifrost_lib::proof::build_block_proof_input;
