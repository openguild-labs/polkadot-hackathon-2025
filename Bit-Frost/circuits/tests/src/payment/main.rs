...
mod tests {
    ...
    use bifrost_core::lp::{encode_liquidity_providers, LiquidityReservation};
    use bifrost_core::payment::{assert_bitcoin_payment, compint_to_u64};
    use bifrost_lib::transaction::{
        build_bifrost_payment_transaction, serialize_no_segwit, P2WPKHBitcoinWallet,
    };
    use bifrost_lib::{load_hex_bytes, to_hex_string};
    ...
    fn assert_theo_btc_payment() {
        ...
        let unbroadcast_txn = build_bifrost_payment_transaction(
            order_nonce,
            &lp_reservations,
            utilized_txid,
            utilized_transaction,
