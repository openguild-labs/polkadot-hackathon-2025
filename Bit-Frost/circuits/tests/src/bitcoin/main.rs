...
mod tests {
    ...
    use bifrost_core::btc_light_client::{
        assert_blockchain, assert_pow, bits_to_target, verify_block, AsLittleEndianBytes,
        Block as bifrostOptimizedBlock,
    };
    use bifrost_lib::transaction::get_chainworks;
    use bifrost_lib::{
        get_retarget_height_from_block_height, load_hex_bytes, AsbifrostOptimizedBlock,
    };
    use crypto_bigint::{Encoding, U256};
    use hex_literal::hex;

    #[test]
    fn test_bifrost_block_converter() {
        ...
        let bifrost_block = block.as_bifrost_optimized_block_unsafe().serialize();
        assert_eq!(
            canon_serialized_header, bifrost_block,
            "bifrost block serialization failed"
        );
    }
    ...
    fn test_block_hash() {
        let block = deserialize::<Block>(&load_hex_bytes("data/block_858564.hex")).unwrap();
        let bifrost_block = &block.as_bifrost_optimized_block_unsafe();

        let canon_block_hash = block.header.block_hash().as_byte_array().to_little_endian();
        let bifrost_block_hash = bifrost_block.compute_block_hash();

        assert_eq!(
            canon_block_hash, bifrost_block_hash,
    ...
    fn test_bits_to_target() {
        ...
        let bifrost_block = &block.as_bifrost_optimized_block_unsafe();
        println!("Bits: {:?}", bifrost_block.bits);

        let canon_target = block.header.target();

        let proposed_target = bits_to_target(bifrost_block.bits);
    ...
    fn test_assert_pow() {
        ...
        assert_pow(
            &bifrost_block.compute_block_hash(),
            bifrost_block,
            bits_to_target(bifrost_block.bits),
        );
    }
    ...
    fn test_assert_pow_fails_on_hash_less_than_target() {
        let block = deserialize::<Block>(&load_hex_bytes("data/block_858564.hex")).unwrap();
        let mut bifrost_block = block.as_bifrost_optimized_block_unsafe();
        //  a nonce that will ~100% likely result in a hash less than the target
        bifrost_block.nonce = [0; 4];
    ...
    fn test_verify_block_transition() {
        ...
        let first_bifrost_block = first_block.as_bifrost_optimized_block_unsafe();
        let second_bifrost_block = &second_block.as_bifrost_optimized_block_unsafe();

        let first_block_hash = first_bifrost_block.compute_block_hash();
        let second_block_hash = second_bifrost_block.compute_block_hash();

        let retarget_height = get_retarget_height_from_block_height(first_bifrost_block.height);
        let retarget_block = deserialize::<Block>(&load_hex_bytes(&format!(
            "data/block_{}.hex",
            retarget_height
        )))
        .unwrap();
        let bifrost_retarget_block = &retarget_block.as_bifrost_optimized_block_unsafe();
        ...
        verify_block(
            second_block_hash,
            first_block_hash,
            second_bifrost_block,
            bifrost_retarget_block,
            first_bifrost_block.height,
        )
    }
    ...
    fn test_verify_block_transition_fails_no_link() {
        ...
        let mut first_bifrost_block = first_block.as_bifrost_optimized_block_unsafe();
        first_bifrost_block.prev_blockhash = [0; 32];
        let second_bifrost_block = &second_block.as_bifrost_optimized_block_unsafe();
        ...
        let blocks = block_heights
            ...
            .map(|height| {
                deserialize::<Block>(&load_hex_bytes(&format!("data/block_{}.hex", height)))
                    .unwrap()
                    .as_bifrost_optimized_block_unsafe()
            })
            .collect::<Vec<bifrostOptimizedBlock>>();
