...
export const Timeline: React.FC<TimelineProps> = ({ children, ...rest }) => {
	...
	useEffect(() => {
		const handleScroll = () => {
			...
			const contentHeight = isMobile ? MOBILE_LINE + listedWallets.length * 100 : 1746;
			let height = 0;
	...
	return (
		<div className="relative">
			...
			<div
				className={styles.line}
				style={{
					height: isMobile ? MOBILE_LINE + listedWallets.length * 100 : 1746,
...
export const TimelineItem: React.FC<TimelineItemProps> = (props) => {
	...
	useEffect(() => {
		const handleScroll = () => {
			...
			const contentHeight = isMobile ? 1570 : 1746;
			let height = 0;
			if (scrollTop - maxHeight > contentHeight) {
				height = contentHeight;
			} else if (scrollTop - maxHeight < 0) {
