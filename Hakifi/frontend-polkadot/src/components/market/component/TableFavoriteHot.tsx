...
export default function TableFavoriteHot({
	...
	const columns: ColumnDef<MarketPair>[] = useMemo(
		() => {
			const column: ColumnDef<MarketPair>[] = [
				{
					accessorKey: "asset",
					header: () => <span className="text-xs text-typo-secondary">Pair</span>,
					cell: (props) => {
						return (
...

				{
					...
					header: () => (
						<span className="text-xs text-typo-secondary">Market Price</span>
					),
					cell: (props) => {
						return (
				...
				},
				{
					...
					header: () => (
						<span className="text-xs text-typo-secondary">24h Change</span>
					),
					cell: (props) => {
				...
				},
				{
					accessorKey: "high",
					header: () => (
						<span className="text-xs text-typo-secondary">24h High</span>
					),
					cell: (props) => {
						return (
							<div className="text-xs text-typo-primary">{`$${formatNumber(
				...
				},
				{
					accessorKey: "low",
					header: () => (
						<span className="text-xs text-typo-secondary">24h Low</span>
					),
					cell: (props) => {
						return (
							<div className="text-xs text-typo-primary">{`$${formatNumber(
...

				{
					accessorKey: "margin",
					header: () => (
						<span className="text-xs text-typo-secondary">Total Margin</span>
					),
					cell: (props) => {
						return (
							<div className="text-xs text-typo-primary">{`$${formatNumber(
			...
			if (type === "favorite") {
				column.unshift({
					...
					header: () => <FavoriteOutlineIcon className="lg:size-6 size-5" />,
					cell: (props) => (
						<div
	...
	if (type === "favorite") {
		columns.splice(2, 0, {
			...
			header: () => <span className="text-xs text-typo-secondary">Category</span>,
			cell: (props) => {
	...
	if (type === "hot") {
		columns.splice(7, 0, {
			...
			header: () => (
				<span className="text-xs text-typo-secondary">Bull / Bear</span>
			),
			cell: (props) => {
