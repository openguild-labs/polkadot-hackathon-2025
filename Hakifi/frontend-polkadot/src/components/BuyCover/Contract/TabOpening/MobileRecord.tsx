...
import { scanUrl, shortenHexString } from '@/utils/helper';
...
const MobileRecord = ({ data, onShowDetail }: RecordProps) => {
    const { asset, token, side, expiredAt, txhash, p_claim, p_liquidation, q_claim, margin, p_cancel, state, unit, p_open, q_cover_pack_id } = useMemo(() => data, [data]);
    const { setInsuranceSelected, toggleCloseModal, plans } = useInsuranceStore();
    ...
    const plan = useMemo(() => plans.find((plan) => plan.id === q_cover_pack_id), [plans, q_cover_pack_id])
    return (
