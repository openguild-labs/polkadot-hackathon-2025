...
export class InsurancesService {
  ...
  async updateTxHash(userId: string, id: string, txhash: string) {
    ...
    try {
      await this.insuranceContractService.getTransactionBlock(txhash);
    } catch (error) {
      throw new BadRequestException('Failed to fetch transaction block');
    } finally {
  ...
  async cancelBinanceOrders(insurance: Insurance) {
    if (
      ![
    ...
    try {
      ...
      console.error('Error cancel binance order', error);
    }
  }
}
