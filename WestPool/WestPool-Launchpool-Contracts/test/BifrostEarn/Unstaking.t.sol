...
contract BifrostStakingTest is Test {
    ...
    function testPartialUnstake() public {
        ...
        uint256 userBalanceBeforeUnstake = vDot.balanceOf(user);
        vm.prank(user);
        mockBifrost.unstake(userStakeAmount - 5 ether);
        ...
        console.log("User final balance:", userFinalBalance, "vDOT");
        (stakedAmount, startTime, pendingReward) = mockBifrost.getStakeInfo(
            user
        );
        console.log("User staked amount after unstake:", stakedAmount, "vDOT");
        console.log("User reward:", pendingReward, "vDOT");
    }

    function testUnstakeAmountOverStakeAmount() public {
        // Ensure initial setup
        uint256 userStakeAmount = 10 ether;
        address user = address(0x123);
    ...
    function testUnstakeWithoutStaking() public {
        ...
        // Unstake without staking
        vm.prank(user);
        vm.expectRevert("Insufficient staked amount");
        mockBifrost.unstake(userStakeAmount);
    }

    function testRepeatedUnstaking() public {
        // Ensure initial setup
        uint256 userStakeAmount = 10 ether;
        address user = address(0x123);
        ...
        for (uint256 i = 0; i < 5; i++) {
            vm.prank(user);
            mockBifrost.unstake(1 ether);
        }
        vm.prank(user);
        mockBifrost.unstake(5 ether);
    }
}
