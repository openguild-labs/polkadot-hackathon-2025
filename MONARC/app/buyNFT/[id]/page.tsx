  ...
  const fetchNFTDetails = useCallback(async () => {
    ...
    try {
      ...
      if (selectedNetwork === 'SEPOLIA') {
        if(!DynamicNFTContract || !IpfsHashStorageContract) return;
        ...
        nftDetails = {
          id,
          title: `NFT #${id}`,
          price: Number(web3?.utils.fromWei(listingPrice, 'ether')),
          priceSymbol: 'ETH',
          owner,
          isListed,
        ...
        nftDetails = {
          id,
          title: `NFT #${id}`,
          price: Number(web3?.utils.fromWei(listing.price, 'ether')),
          priceSymbol: 'REACT',
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <Alert variant="destructive">
          ...
          <AlertDescription>The requested NFT could not be found. Please check the ID and try again.</AlertDescription>
        </Alert>
      </div>
    )
  }
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {parseFloat(userBalance) < Number(nft.price) && selectedNetwork === 'KOPLI' && (
                <div className="space-y-2">
                  <Alert variant="destructive">
                    <AlertCircle className="h-4 w-4" />
        ...
        <CardFooter className="flex flex-col space-y-4">
          ...
          <Button 
            ...
            disabled={!nft.isListed || parseFloat(userBalance) < Number(nft.price) || transactionPending || (selectedNetwork === 'KOPLI' && !isApproved)}
            className="w-full"
          >
