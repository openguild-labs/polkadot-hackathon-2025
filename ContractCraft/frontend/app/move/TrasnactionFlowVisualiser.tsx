...
const TransactionFlowVisualizer: React.FC<TransactionFlowVisualizerProps> = ({
      ...
      if (provider) {
        try {
          ...
          if (accounts[0]) {
            ...
            addLog(
              `Connected to wallet: ${accounts[0].slice(0, 8)}...`,
              "success"
            );
          }
          ...
          provider.on('accountChanged', (accounts: string[]) => {
            if (accounts.length > 0) {
              setWalletAddress(accounts[0]);
              addLog(
                `Wallet account changed: ${accounts[0].slice(0, 8)}...`,
                "info"
              );
            } else {
        ...
        addLog("StarKey wallet not found", "error");
      }
    };
  ...
  const getFlowStatus = (
    ...
    if (lastBlock.category === "contract") {
      return {
        message:
          "This operation interacts with a Supra(MOVE) smart contract. Please verify the details.",
        type: "warning",
      };
    }
              ...
              >
                ...
                <span className="font-mono">
                  {Object.entries(value).map(([k, v]) => (
                    <div key={k}>
                      {k}: {v} {k.toLowerCase().includes("amount") ? "SUPRA" : ""}
                    </div>
                  ))}
                </span>
              </div>
